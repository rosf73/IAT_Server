  ...
  if (err) {
    ...
    db.run(createParticipantQuery, (err) => {
      if (err) { /** Already created Table */ }
        ...
        db.run(insertParticipantQuery, [md5("01012345678"), 18, 2, 22, "none"]);
        db.run(insertParticipantQuery, [md5("01066775544"), 18, 1, 22, "none"]);
        db.run(insertParticipantQuery, [md5("01087654321"), 21, 1, 31, "none"]);
        db.run(insertParticipantQuery, [md5("01075757575"), 50, 2, 40, "컴퓨터공학"]);
      }
    });
