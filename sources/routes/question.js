  ...
  const map_result = await Promise.all(
    result.map(async elem => {
      ...
      if (elem.type === 1) { // base type
        const case_result = await db.query(`SELECT * FROM base_case WHERE question_id = ${elem.question_id} ORDER BY number ASC`, []);
      ...
      if (elem.type === 2) { // table type
        const sub_question_result = await db.query(`SELECT content FROM sub_question WHERE question_id = ${elem.question_id} ORDER BY number ASC`, []);
        ...
        const table_case_result = await db.query(`SELECT content FROM table_case WHERE question_id = ${elem.question_id} ORDER BY number ASC`, []);
      ...
      if (elem.type === 3) { // listed_type
        const sub_question_result = await db.query(`SELECT sub_question_id, content FROM sub_question WHERE question_id = ${elem.question_id} ORDER BY number ASC`, []);
        const map_sub_question_result = await Promise.all(
          sub_question_result.map(async e => {
            const sub_case_result = await db.query(`SELECT content FROM sub_case WHERE sub_question_id = ${e.sub_question_id} ORDER BY number ASC`, []);
      ...
      if (elem.type === 4) { // IAT type
        const sub_question_result = await db.query(`SELECT sub_question_id, number, content FROM sub_question WHERE question_id = ${elem.question_id} ORDER BY number ASC`, []);
        const map_sub_questions = await Promise.all(
          sub_question_result.map(async ele => {
            const steps_result = await db.query(`SELECT * FROM test_step WHERE sub_question_id = ${ele.sub_question_id} ORDER BY step ASC`, []);
            const word_set = [];
            ...
            ele.words = word_set;
            ele.steps = map_steps_result;
            delete ele.sub_question_id;
            return ele;
          })
        );
        elem.sub_contents = map_sub_questions;
      }
